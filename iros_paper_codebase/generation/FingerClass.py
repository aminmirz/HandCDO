...
class FingerCode2Chain:
    def __init__(self, finger_code: str, link_added_length_mm_list: list = None):
        # finger codes are In the format of xxx - yy - zz zz zz zz zz
        '''
        xxx is the rotation mode of the joint
        ------ x can be 0 or 1
        000: no rotation
        100: vertical axis rotation
        101: horizontal axis rotation - short
        110: horizontal axis rotation - long
        111: verical + horizontal axis rotation

        yy is the number of joints before and after the rotation joint
        ------ y can be 0, 1, 2, 3, 4, 5
        00: 0 joints before and after
        01: 0 joint before and 1 joint after
        10: 1 joint before and 0 joints after
        11: 1 joints before and 1 joint after

        zz zz zz zz zz is the before/after joint types
        ------ z can be 0 or 1
        01: short joint
        10: long joint
        '''
        ...
        self.rotation_chain = {
            '000':[],
            '100':[link(3), joint(3,rotation_range_rad=range_list[3])],
            '101':[link(2), joint(2,rotation_range_rad=range_list[2]), joint(5,rotation_range_rad=range_list[5])],
            '110':[link(1), joint(2,rotation_range_rad=range_list[2]), joint(6,touple_idx=1,rotation_range_rad=range_list[6])],
            '111':[link(2), joint(2,rotation_range_rad=range_list[2]),joint(3,rotation_range_rad=range_list[3])]
            }[self.finger_code[0:3]]

        self.assemble_joints()
        self.add_fingertip()

    def add_joint(self, length_code: str):
        range_list = {4:(-0.2,1.7), 7:(-0.5,1.7)}
        return {
            '01':[joint(4,rotation_range_rad=range_list[4])],
            '10':[joint(7,rotation_range_rad=range_list[7])],
            }[length_code]
    ...
    def assemble_joints(self):
        ...
        n_before = int(self.finger_code[4])
        n_after = int(self.finger_code[5])
        chain = []
        for i in range(n_before):
            chain.extend(self.add_link())
            element = self.add_joint(str(self.finger_code[7+i*2:7+(i+1)*2]))
        ...
        for i in range(n_after):
            chain.extend(self.add_link())
            chain.extend(self.add_joint(str(self.finger_code[7+n_before*2+i*2:7+n_before*2+(i+1)*2])))
...
class ThumbCode2Chain:
    def __init__(self, thumb_code: str, link_added_length_mm_list: list = None, side: str = 'left'):
        # r-a-zzzzzzzz (r ∈ {0,1}, a = total joints to add, z pairs describe joint types)
        self.link_added_length_mm_list = link_added_length_mm_list or []
        self._add_link_counter = 0
        self.side = side
        self.chain_elements = []
        ...
        # Rotation chain for thumb:
        # '0' => no rotation ny base rotation, '1' => add secndary rotation joint
    ...
    def assemble_joints(self):
        # Format: r-a-zzzzzzzz..., so count at index 2, pairs start at index 4
        n_total = int(self.thumb_code[2])
        chain = []
        chain.extend(self.rotation_chain)
        for i in range(n_total):
            chain.extend(self.add_link())
            code_pair = str(self.thumb_code[4 + i * 2: 4 + (i + 1) * 2])
            chain.extend(self.add_joint(code_pair))
        self.chain_elements = chain
        # print(self.chain_elements)
