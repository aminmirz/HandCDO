...
class Hand:
    ...
    def __init__(
        ...
        self.finger_bases = np.concatenate((self.palm.cfg.finger_bases, np.zeros((self.palm.cfg.finger_number, 1))), axis=1)
        self.finger_bases_normal_vectors = np.concatenate((self.palm.cfg.finger_bases_normal_vectors, np.zeros((self.palm.cfg.finger_number, 1))), axis=1)
        self.thumb_bases = np.concatenate((self.palm.cfg.thumb_bases, np.zeros((self.palm.cfg.thumb_number, 1))), axis=1)
        self.thumb_bases_normal_vectors = np.concatenate((self.palm.cfg.thumb_bases_normal_vectors, np.zeros((self.palm.cfg.thumb_number, 1))), axis=1)
    ...
    def save_assembly_data(self):
        ...
        try:
            ...
            np.savez(
                ...
                finger_pad_resolution_level=int(self.finger_cfgs[0].pad_resolution_level),
                finger_bump_edge_zeroing_spread=float(self.finger_cfgs[0].bump_edge_zeroing_spread),
    ...
    def save_palm_data(self):
        ...
        def _signed_area(points: np.ndarray) -> float:
            """Compute signed area of a closed or open polygon (XY)."""
        ...
        def _line_segment_intersection(a0: np.ndarray, a1: np.ndarray, b0: np.ndarray, b1: np.ndarray, tol: float = 1e-9):
        ...
        def _resolve_self_intersections(vertices: np.ndarray,
                                        offset_mm: float,
                                        original_area: float,
                                        cyclic: bool = True) -> np.ndarray:
            """
            Resolve self intersections of an offset polygon, following the same
            topological clean-up as offsetPolygonOfSpline in _curve_offset_toolbox.
            """
            ...
            while i < len(verts):
                j = i + 2
                # Note: last segment wraps to index 0 when cyclic
                while j < len(verts) - (0 if i > 0 else 1):
                    inter = _line_segment_intersection(verts[i - 1], verts[i],
                                                    verts[j - 1], verts[j])
                    if inter is None:
                        j += 1
                        continue

                    # Midpoint between the two intersection points (they are equal here)
                    inter_pt = 0.5 * (inter[2] + inter[3])

                    # areaInner and areaOuter as in the Blender script
                    tri_inner = np.vstack([inter_pt, verts[i], verts[j - 1]])
                    tri_outer = np.vstack([inter_pt, verts[j], verts[i - 1]])
                    area_inner = sign * _signed_area(tri_inner)
                    area_outer = sign * _signed_area(tri_outer)
                    ...
                    if area_inner > area_outer:
                        ...
                        # Clip away the inner loop
                        verts = verts[:i] + [inter_pt] + verts[j:]
                        j = i + 2
                i += 1

            new_area = _signed_area(np.asarray(verts, dtype=float))
            # If orientation flipped, reverse to match original
            if original_area * new_area < 0.0:
                verts = list(reversed(verts))

            return np.asarray(verts, dtype=float)

        def _offset_polygon_inward(points: np.ndarray, offset_mm: float) -> np.ndarray:
            """
            Compute a single inward offset of a simple 2D polygon using line-offset
            intersections (similar in spirit to _curve_offset_toolbox, but Blender-free).

            points: Nx2 polygon vertices in mm, assumed ordered around the shape.
            offset_mm: positive offset distance in mm (inward).
            """
            pts = np.asarray(points, dtype=float)
            n = pts.shape[0]
            ...
            # Determine orientation; we need inward normals.
            ...
            def _edge_inward_normal(p0: np.ndarray, p1: np.ndarray) -> np.ndarray:
                t = _unit(p1 - p0)
                if ccw:
                    # CCW polygon: outward normal is (t_y, -t_x); inward is opposite.
                    n_out = np.array([t[1], -t[0]], dtype=float)
                else:
                    # CW polygon: outward normal is (-t_y, t_x); inward is opposite.
                    n_out = np.array([-t[1], t[0]], dtype=float)
                n_in = -_unit(n_out)
                return n_in
            ...
            for i in range(n):
                ...
                # Previous edge: p_prev -> p_curr
                t_prev = _unit(p_curr - p_prev)
                n_prev_in = _edge_inward_normal(p_prev, p_curr)
                a1 = p_prev + n_prev_in * offset_mm

                # Current edge: p_curr -> p_next
                t_curr = _unit(p_next - p_curr)
                n_curr_in = _edge_inward_normal(p_curr, p_next)
                b1 = p_curr + n_curr_in * offset_mm

                # Intersection of the two offset lines:
                # a1 + s * t_prev = b1 + t * t_curr
                denom = _cross2(t_prev, t_curr)
                if abs(denom) < 1e-9:
                    # Nearly parallel; fall back to average of shifted positions.
                    q_prev = p_curr + n_prev_in * offset_mm
                    q_curr = p_curr + n_curr_in * offset_mm
                    q = 0.5 * (q_prev + q_curr)
                else:
            ...
            # Final clean-up step: resolve self-intersections like the guide code
            new_pts_arr = _resolve_self_intersections(new_pts_arr, offset_mm=offset_mm, original_area=area, cyclic=True)
            return new_pts_arr

        def compute_inward_offset_recursive(points: np.ndarray, total_offset_mm: float, steps: int = 1) -> np.ndarray:
            """
            Apply the inward offset in several recursive (iterative) steps, which
            is more stable on shapes with tighter concavities.
            """
            pts = np.asarray(points, dtype=float)
            if steps <= 1:
                return _offset_polygon_inward(pts, total_offset_mm)
            step = float(total_offset_mm) / float(steps)
            for _ in range(steps):
                pts = _offset_polygon_inward(pts, step)
            return pts
        ...
        def plot_palm_outlines(outline: np.ndarray,
                               inward_offset_outline: np.ndarray,
                               thumb_squares: list,
            ...
            plt.figure()
            plt.plot(outline[:, 0], outline[:, 1], "k-o", label="Original outline")
            plt.plot(inward_offset_outline[:, 0], inward_offset_outline[:, 1], "r-o", label="Inward offset outline")
        ...
        try:
            ...
            inward_offset_outline = compute_inward_offset_recursive(self.palm.get_outline(), total_offset_mm=self.hand_cfg.palm_wall_thickness_mm, steps=10)
            ...
            if Plots().palm_outline_plot_2d:
                plot_palm_outlines(
                    self.palm.get_outline(), inward_offset_outline, thumb_squares,
            ...
            np.savez(
                ...
                outline=self.palm.get_outline(),
                inward_offset_outline=inward_offset_outline,
