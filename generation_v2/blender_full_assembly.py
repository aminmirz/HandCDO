...
from utils import outline_is_valid_inner
...
class PalmMesh:
    def __init__(self, data_path: str):
        ...
        self.outline = np.array(self.data.get("outer_outline", self.data["outline"]), dtype=float)
        self.inner_outline = np.array(self.data.get("inner_outline", self.data.get("inward_offset_outline", self.outline)), dtype=float)
        if not outline_is_valid_inner(self.inner_outline, self.outline):
            raise ValueError(
                f"Invalid inner palm outline in {data_path}. Regenerate the hand so "
                "the cavity is computed from the final modified palm outline."
            )
        self.inward_offset_outline = self.inner_outline
        ...
        self.palm_pad_thickness_m = float(self.data.get("palm_pad_thickness_mm", 1.0)) * mm_to_m
    ...
    def _resample_closed_loop(points: np.ndarray, count: int) -> np.ndarray:
        ...
        if pts.ndim != 2 or pts.shape[1] != 2 or pts.shape[0] < 3 or count <= 0:
            return pts
        ...
        seg_len = np.linalg.norm(nxt - pts, axis=1)
        ...
        seg_idx = 0
        for s in samples:
            while seg_idx < len(seg_len) - 1 and cumulative[seg_idx + 1] <= s:
                seg_idx += 1
            denom = max(seg_len[seg_idx], 1e-12)
            t = (s - cumulative[seg_idx]) / denom
            out.append((1.0 - t) * pts[seg_idx] + t * pts[(seg_idx + 1) % pts.shape[0]])
        return np.asarray(out, dtype=float)

    def create_main_extruded_mesh(self):
        """
        create a 3D palm main body mesh in Blender
        by extruding the main outline, then cutting the inside with the
        inward-offset outline using the original generation boolean workflow.
        """
        ...
        # Create palm pad from the configured pad thickness along world +Z.
        if bpy.data.objects.get(self.pad_name) is None:
            ...
            if extruded_pad:
                bmesh.ops.translate(pad_bm, verts=extruded_pad, vec=(0.0, 0.0, self.palm_pad_thickness_m))
    ...
    def remove_inside_mesh(self):
        """
        make a boolean operation to remove the inside of the main extruded mesh by the inner outline
        """
        # Validate inner outline
        inner = np.asarray(self.inner_outline, dtype=float)
        if inner.ndim != 2 or inner.shape[1] != 2 or inner.shape[0] < 3:
            print("No valid inner outline available for boolean cut.")
            return
        ...
        # Create cutter mesh from inner outline
        bm = bmesh.new()
        verts = []
        bottom_margin_m = 0.5 * mm_to_m
        for x_mm, y_mm in inner:
            v = bm.verts.new((float(x_mm) * mm_to_m, float(y_mm) * mm_to_m, -bottom_margin_m))
            verts.append(v)
        ...
        # Extrude cutter through the bottom face and up to the same cavity ceiling.
        height_m = self.palm_extrude_height_m - self.palm_thickness_m
        res = bmesh.ops.extrude_face_region(bm, geom=[face])
        geom_extruded = res.get("geom", [])
        extruded_verts = [elem for elem in geom_extruded if isinstance(elem, bmesh.types.BMVert)]
        if extruded_verts:
            bmesh.ops.translate(bm, verts=extruded_verts, vec=(0.0, 0.0, height_m + bottom_margin_m))
    ...
    def add_screw_holes(self):
        ...
        for idx, attach_obj in enumerate(attach_objects):
            # Duplicate through the data API, not bpy.ops.object.duplicate().
            # In addon mode the template may live in the hidden/excluded
            # _handgen_templates collection, so selecting it can fail with
            # "object can't be selected because it is not in ViewLayer".
            screw_holes_copy = screw_holes_template.copy()
            if screw_holes_template.data is not None:
                screw_holes_copy.data = screw_holes_template.data.copy()
            screw_holes_copy.name = f"screw_holes_{idx}"
            bpy.context.scene.collection.objects.link(screw_holes_copy)
    ...
    def add_screw_attachment(self):
        ...
        **we move the cylinder centers 1 mm closer to the origin.
        ...
        inner_bottom_margin_m = 0.5 * mm_to_m
        for idx, loc_mm in enumerate(self.cylinder_locations):
            inner_cut_depth_m = inner_height_m + inner_bottom_margin_m
            loc_m = (
                float(loc_mm[0]) * mm_to_m,
                float(loc_mm[1]) * mm_to_m,
                (inner_height_m - inner_bottom_margin_m) / 2.0,
            )

            bpy.ops.mesh.primitive_cylinder_add(
                radius=inner_diameter_m / 2.0,
                depth=inner_cut_depth_m,
                location=loc_m
            )
            cyl = bpy.context.active_object
...
class FingerAssembly:
    def __init__(self, data_path: str, template_objects=None, open_main_file: bool = True):
        self.data_path = data_path
        self.elements = []
        self.data = np.load(data_path, allow_pickle=True)
        ...
        self.finger_pad_resolution_levels = self.data.get("finger_pad_resolution_levels", None)
        self.thumb_pad_resolution_levels = self.data.get("thumb_pad_resolution_levels", None)
        self.finger_pad_thickness_mm = float(self.data.get("finger_pad_thickness_mm", 0.5))
        self.finger_pad_thickness_mm_list = self.data.get("finger_pad_thickness_mm_list", None)
        self.thumb_pad_thickness_mm_list = self.data.get("thumb_pad_thickness_mm_list", None)
        ...
        if open_main_file:
            ...
            try:
                ...
                return
            template_objects = [o for o in bpy.data.objects]

        self.set_template_objects(template_objects or [])

    @classmethod
    def from_existing_scene(cls, data_path: str, template_objects):
        """Build a FingerAssembly using already appended template objects.

        This is used by the Blender add-on so generating a hand does not replace
        the user's current .blend file with components.blend.
        """
        return cls(data_path, template_objects=template_objects, open_main_file=False)

    def set_template_objects(self, template_objects):
        # Use template objects from components.blend. We'll duplicate them and keep originals intact.
        self.all_objs = list(template_objects)
                ...
                try:
                    ...
                    if outline_mm.size > 0:
                        ...
                        if joint_root is not None:
                            ...
                            if pts_m.shape[0] >= 3:
                                pad_thickness_mm = self.finger_pad_thickness_mm
                                try:
                                    thicknesses = self.thumb_pad_thickness_mm_list if is_thumb else self.finger_pad_thickness_mm_list
                                    if thicknesses is not None and finger_idx < len(thicknesses):
                                        pad_thickness_mm = float(thicknesses[finger_idx])
                                except Exception:
                                    pad_thickness_mm = self.finger_pad_thickness_mm
                                extrude_vec = (0.0, 0.0, pad_thickness_mm * mm_to_m)
                                ...
                                try:
                                    ...
                                    rem = pad_obj.modifiers.new(name="PadRemesh", type="REMESH")
                                    rem.mode = 'SHARP'
                                    res_level = self.finger_pad_resolution_level
                                    try:
                                        levels = self.thumb_pad_resolution_levels if is_thumb else self.finger_pad_resolution_levels
                                        if levels is not None and finger_idx < len(levels):
                                            res_level = int(levels[finger_idx])
                                    except Exception:
                                        res_level = self.finger_pad_resolution_level
                                    rem.octree_depth = int(res_level)
                                    rem.use_smooth_shade = True
                                    bpy.ops.object.modifier_apply(modifier=rem.name)
                                except Exception as e:
...
def assign_viz_materials():
    ...
    finger_mats = sorted(
        [m.name for m in bpy.data.materials if m.name.startswith("f") and m.name[1:].isdigit()],
    ...
    for sub_col in hand_col.children:
        ...
        if col_name.startswith("finger_"):
            n = int(col_name.split("_")[-1])  # "1", "2", "3", ...
            base_mat = finger_mats[(n - 1) % len(finger_mats)] if finger_mats else "black"
...
def shade_all_meshes_flat():
    """Ensure the saved Blender file uses flat shading for every mesh object."""
    count = 0
    for obj in bpy.data.objects:
        ...
        obj.data.update()
        count += 1
    print(f"[shade] Set flat shading on {count} mesh object(s).")
...
class ExportURDF:
    ...
    def material_urdf_info(self, obj):
        """Return a URDF-safe material name and RGBA string for a visual object."""
        import re

        mat = obj.material_slots[0].material if obj.material_slots and obj.material_slots[0].material else None
        if mat is None:
            return None
        name = re.sub(r"\.\d+$", "", mat.name.strip())
        name = re.sub(r"[^A-Za-z0-9_]+", "_", name).strip("_")
        if not name:
            name = "material"
        rgba = getattr(mat, "diffuse_color", (0.8, 0.8, 0.8, 1.0))
        rgba = [max(0.0, min(1.0, float(c))) for c in rgba[:4]]
        return name, " ".join(f"{c:.9g}" for c in rgba)
    ...
    def write_urdf(self, path, robot_name, links, joints, package_name):
        ...
        for L in links:
            ...
            for visual in L.get("visuals", []):
                if len(visual) == 4:
                    mesh_rel, xyz, rpy, mat_info = visual
                else:
                    mesh_rel, xyz, rpy = visual
                    mat_info = None
                lines.append('    <visual>')
                ...
                if mat_info is not None:
                    mat_name, rgba = mat_info
                    lines.append(f'      <material name="{mat_name}"><color rgba="{rgba}"/></material>')
                lines.append('    </visual>')
            if self.collision_mesh:
    ...
    def export_viz_and_colliders_into_link(self, link_dict, main_obj, only_these_viz=None, scale_suffix=""):
        ...
        for vp in viz_parents:
            for v in self.children_with_key(vp, "viz_", mesh_only=True):
                ...
                link_dict["visuals"].append((rel, (t.x, t.y, t.z), r, self.material_urdf_info(v)))
                seen_vis.add(v)
