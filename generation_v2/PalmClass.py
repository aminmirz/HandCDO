...
class PalmOutline:
    ...
    def construct_finger_base_vectors_and_normals(self):
        # finger_points are already shifted along the outline by side_offset_mm.
        # Base construction applies normal offset at that shifted point, then angle.
        ...
        for i, p in enumerate(self.cfg.finger_points):
            ...
            bases.append(p + n_out * self.cfg.finger_base_normal_offset_mm_list[i])
    ...
    def construct_thumb_base_frames_segments(self):
        ...
        for i, p in enumerate(self.cfg.thumb_points):
            ...
            # Original generation behavior: thumb side offset is a straight
            # local tangent translation, not an arc-length shift on the outline.
            t_hat = np.array([-normal_vectors[i][1], normal_vectors[i][0]], dtype=float)
            t_hat = t_hat / np.linalg.norm(t_hat)
            side_offset = self.cfg.thumb_base_side_offset_mm_list[i]
            seg1_margin_point = seg1_margin_point + t_hat * side_offset
            seg1_point = seg1_point + t_hat * side_offset
            corner = corner + t_hat * side_offset
            seg2_point = seg2_point + t_hat * side_offset
            seg2_margin_point = seg2_margin_point + t_hat * side_offset
            base = base + t_hat * side_offset
